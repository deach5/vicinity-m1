...
(function($)
{
	$.fn.aviaSlider= function(variables) 
	{
		...
		return this.each(function()
		{
			...
			slideWrapper.methods = {
				...
				{	
					...
					{	
						...
						{	
							var controller = $('<a href="javascript:void(0);" class="ie6fix '+current_class+'"></a>').appendTo(controlls);
				...
				{	
					if(options.showText)
					{
						...
						{	
							...
							if(description != "")
							{
                //var cont = currentSlide.has('a') ? currentSlide.find('a') : currentSlide;
                var cont = currentSlide;
          ...
          sliderPrimed()
          //slideWrapper.aviaSlider_callbackhelper({callback:sliderPrimed});
          ...
          slideWrapper.aviaSlider_callbackhelper({callback:switchBegin});
					...
					{	
						...
						var currentSlide = slides.filter(':visible'),
							...
							if(options.slideControlls != 'none')
							{	
                  ...
                  //hack to hide overlay
                  if(i+1 == blockNumber - 10){
                   // $('#overlay_slider').fadeOut()
                  }	
...
(function($){
  $.fn.aviaSlider_callbackhelper = function(callback) {
    var defaults = {callback: ''};
   // var options = $.extend(defaults, variables);
    return this.each(function(){
      var methods = {
        callback: function(){	
         // alert('in callbackhelper == '+callback)
